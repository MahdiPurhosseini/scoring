...
//    public function VerifyLookup($receptor, $token, $token2, $token3 , $template, $type = null)
    public function VerifyLookup($receptor, $token, $template, $type = null)
    {
...
//            "token2" => $token2,
    ...
    public function VerifyLookupContact($receptor, $token, $token10, $template, $type = null)
    {
        $path = $this->get_path("lookup", "verify");
        $params = array(
            "receptor" => $receptor,
    ...
    public function VerifyLookupRegisterConference($receptor, $token, $token10, $token20, $template, $type = null)
    {
        $path = $this->get_path("lookup", "verify");
        $params = array(
            "receptor" => $receptor,
        ...
        if (func_num_args() > 6) {
            $arg_list = func_get_args();
            if (isset($arg_list[7]))
                $params["token10"] = $arg_list[7];
            if (isset($arg_list[8]))
                $params["token20"] = $arg_list[8];
        }
        return $this->execute($path, $params);
    }
